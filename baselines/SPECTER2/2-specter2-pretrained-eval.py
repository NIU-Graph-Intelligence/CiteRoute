"""
SPECTER2 (Pretrained) — Evaluation Script
============================================
Evaluates the official pretrained SPECTER2 (base + proximity adapter)
...
RESULTS_DIR = OUTPUT_DIR / "dense/SPECTER2-pretrained/evaluation_results/"
...
def main():
    log("=" * 80)
    log("SPECTER2 (Pretrained) — Evaluation")
    log("=" * 80)
    ...
    with open(RESULTS_DIR / "specter2_pretrained_metrics.txt", "w") as f:
        f.write("SPECTER2 (Pretrained: allenai/specter2_base + proximity adapter) — No finetuning\n\n")
        for key, cfg in LABEL_TYPES.items():
    ...
    with open(RESULTS_DIR / "specter2_pretrained_metrics.json", "w") as f:
        json.dump({"model": "allenai/specter2_base + allenai/specter2 (proximity)",
                    "type": "pretrained (no finetuning)", **all_results}, f, indent=2)
