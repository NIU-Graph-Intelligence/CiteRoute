"""
SPECTER (Pretrained) — Evaluation Script
==========================================
Evaluates the official pretrained SPECTER model on all 3 label types.
FAISS cosine retrieval from the candidate pool.
"""
...
RESULTS_DIR = OUTPUT_DIR / "dense/SPECTER-pretrained/evaluation_results/"
...
def main():
    log("=" * 80)
    log("SPECTER (Pretrained) — Evaluation")
    log("=" * 80)
    ...
    # Save
    with open(RESULTS_DIR / "specter_pretrained_metrics.txt", "w") as f:
        f.write("SPECTER (Pretrained: allenai/specter) — No finetuning on our data\n\n")
        for key, cfg in LABEL_TYPES.items():
    ...
    with open(RESULTS_DIR / "specter_pretrained_metrics.json", "w") as f:
        json.dump({"model": "allenai/specter", "type": "pretrained (no finetuning)", **all_results}, f, indent=2)
